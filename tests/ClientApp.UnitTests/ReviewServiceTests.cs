...
public class ReviewServiceTests
{
    ...
    {
        var item = new ReviewItemDto(Guid.NewGuid(), 1, "u1", 5, "t", "c", DateTime.UtcNow);
        ...
        var result = await svc.SubmitAsync(new SubmitReviewRequest(1, 5, "t", "c"));
    ...
    {
        var page = new PagedResult<ReviewItemDto>([new ReviewItemDto(Guid.NewGuid(), 2, "u2", 4, "t2", "c2", DateTime.UtcNow)], 1, 1, 10);
    ...
    {
        var agg = new ReviewAggregateDto(3, 4.5);
        var resp = new HttpResponseMessage(HttpStatusCode.OK)
        {
